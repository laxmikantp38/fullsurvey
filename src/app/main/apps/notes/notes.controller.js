(function() {
    ...
    function NotesController($document, $timeout,$rootScope, $scope, $http, $mdSidenav, NotesService, LabelsService, $mdDialog, environment) {
        ...
        function deleteLabel(ev, label) {
            var confirm = $mdDialog.confirm().title('Are you sure want to delete the label?').htmlContent('The "' + label.name + '" label will be deleted permanently.').ariaLabel('delete label').targetEvent(ev).ok('OK').cancel('CANCEL');
